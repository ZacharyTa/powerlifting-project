...
const getWeightDivID = async (sex, weight, age) => {
  const weightDivPath = path.join(__dirname, 
    '..', '..', '..', '..', 
  ...
  return new Promise((resolve) => {
    ...
    fs.createReadStream(weightDivPath)
      ...
      .on('data', (row) => {
        if (
          weight >= parseFloat(row.min_weight) && weight < parseFloat(row.max_weight) &&
          sex === parseInt(row.sex) && isYouth === parseInt(row.is_youth)
        ) {
